...
const adaptOpenAlexWork = (work) => {
  ...
  const authors_list = Array.isArray(work.authorships)
    ? work.authorships
        .map((auth) => {
          const idRaw = auth?.author?.id;
          const idShort = (idRaw || "").split("/").pop();
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-8'>
          ...
          <div className='mb-3 sm:mb-4'>
            <p className='text-sm sm:text-base md:text-lg text-white/90'>
              ...
              {Array.isArray(paper.authors_list) &&
              paper.authors_list.length > 0
                ? paper.authors_list.map((a, idx) => (
                    <span key={(a.id || a.name) + idx}>
                      ...
                      {idx < paper.authors_list.length - 1 ? ", " : ""}
                    </span>
                  ))
                : paper.authors || "N/A"}
            </p>
              ...
              <div className='mb-6 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600'>
                ...
                {paper.biblio?.first_page && (
                  <div>
                    ...
                    {paper.biblio.first_page}
                    {paper.biblio.last_page ? `-${paper.biblio.last_page}` : ""}
                  </div>
                )}
                {paper.source_issn_l && (
                  <div>
                ...
                {paper.concepts?.length > 0 ? (
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    {paper.concepts.map((c) => {
                      const pct =
                        Math.max(0, Math.min(1, Number(c.score || 0))) * 100;
                      return (
                        <div
                          key={c.id || c.name}
