...
import { getOpenAlexWorks, getOpenAlexFilters } from "../../utils/api";
        ...
        {visibleItems.map((item, idx) => {
          const label =
            item && typeof item === "object"
              ...
              : String(item ?? "");
          const value =
            item && typeof item === "object" ? item.value ?? label : label;
          const count =
            item && typeof item === "object"
              ? item.count ?? item.works_count ?? null
              : null;
...
const adaptPapers = (openAlexData) => {
  ...
  return items.map((work) => {
    ...
    const yearNum = Number(work?.publication_year || 0) || 0;
    const citedByCount = Number(work?.cited_by_count || 0) || 0;
    const fulltextUrl =
...
export default function Paper() {
  ...
  const [filtersData, setFiltersData] = useState({ sdg: [], concepts: [] });
  ...
  const buildFilterParam = (
    ...
    if (Array.isArray(sdgs) && sdgs.length > 0) {
      parts.push(`sdg:${sdgs.join("|")}`);
    }
    if (Array.isArray(concepts) && concepts.length > 0) {
      parts.push(`concept:${concepts.join("|")}`);
    }
    if (fromYear) {
  ...
  const parseFilterParam = (filterStr) => {
    ...
    for (const p of parts) {
      ...
      } else if (p.startsWith("sdg:")) {
        const raw = p.slice("sdg:".length);
        next.sdgs = raw.split("|").filter(Boolean);
      } else if (p.startsWith("concept:")) {
        const raw = p.slice("concept:".length);
        next.concepts = raw.split("|").filter(Boolean);
  ...
  const applyFiltersToUrl = (
    ...
    const filterStr = buildFilterParam(types, fromYear, toYear, sdgs, concepts);
  ...
  const toggleSdg = (name) => {
    const exists = selectedSdgs.includes(name);
    const next = exists
      ? selectedSdgs.filter((v) => v !== name)
      : [...selectedSdgs, name];
  ...
  const toggleConcept = (name) => {
    const exists = selectedConcepts.includes(name);
    const next = exists
      ? selectedConcepts.filter((v) => v !== name)
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const q = searchParams.get("q") || "";
        const filter = searchParams.get("filter") || "";
        const data = await getOpenAlexFilters({ limit: 200, q, filter });
        setFiltersData({
          sdg: data?.sdgs || [],
          concepts: data?.concepts || [],
        });
      } catch (e) {
        setFiltersData({ sdg: [], concepts: [] });
      }
    };
    fetchFilters();
  }, [searchParams]);
  ...
  useEffect(() => {
    const { sdgs, concepts } = parseFilterParam(
      searchParams.get("filter") || ""
    );
  ...
  const startIndex =
    sortedPapers.length > 0
      ? (meta.page - 1) * (perPage || sortedPapers.length) + 1
      : 0;
  const endIndex =
    sortedPapers.length > 0
      ? Math.min(
          startIndex + sortedPapers.length - 1,
          Number(meta.count) || startIndex + sortedPapers.length - 1
        )
      : 0;
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8'>
          ...
          >
            ...
            <div className='mb-4'>
              ...
              >
                ...
                <option value='cite-desc'>Citations (Most)</option>
                <option value='cite-asc'>Citations (Least)</option>
              </select>
            </div>

            <SidebarSection
              title='Sustainable Development Goals'
              items={filtersData.sdg || []}
              onToggle={(v) => toggleSdg(v)}
              isChecked={(v) => selectedSdgs.includes(v)}
              onShowMore={() => setShowSdgModal(true)}
            />
            <SidebarSection
              title='Concepts'
              items={filtersData.concepts || []}
              onToggle={(v) => toggleConcept(v)}
              isChecked={(v) => selectedConcepts.includes(v)}
              onShowMore={() => setShowConceptModal(true)}
            />
            <SidebarSection
              title='Type'
        ...
        {showSdgModal && (
          <div className='fixed inset-0 z-50 flex items-center justify-center'>
            ...
            ></div>
            <div className='relative bg-white rounded-lg shadow-lg max-w-lg w-full max-h-[80vh] overflow-hidden'>
              <div className='flex items-center justify-between p-4 border-b'>
                <h3 className='font-semibold'>All SDGs</h3>
                <button className='p-1' onClick={() => setShowSdgModal(false)}>
                  <span className='material-symbols-outlined'>close</span>
                </button>
              </div>
              <div className='p-4 overflow-auto max-h-[65vh]'>
                <ul className='space-y-2 text-sm'>
                  {(filtersData.sdg || []).map((item, idx) => (
                    <li key={idx}>
                      <label className='flex items-center gap-2 cursor-pointer'>
                        <input
                          type='checkbox'
                          checked={selectedSdgs.includes(item.name)}
                          onChange={() => toggleSdg(item.name)}
              ...
              <div className='p-4 border-t flex justify-end'>
                <button
                  className='px-4 py-2 bg-[#d52727] text-white rounded'
                  onClick={() => setShowSdgModal(false)}
                >
                  Close
                </button>
        ...
        {showConceptModal && (
          <div className='fixed inset-0 z-50 flex items-center justify-center'>
            ...
            <div className='relative bg-white rounded-lg shadow-lg max-w-lg w-full max-h-[80vh] overflow-hidden'>
              <div className='flex items-center justify-between p-4 border-b'>
                <h3 className='font-semibold'>All Concepts</h3>
                <button
                  className='p-1'
                  onClick={() => setShowConceptModal(false)}
                >
              ...
              <div className='p-4 overflow-auto max-h-[65vh]'>
                <ul className='space-y-2 text-sm'>
                  {(filtersData.concepts || []).map((item, idx) => (
                    <li key={idx}>
                      <label className='flex items-center gap-2 cursor-pointer'>
                        <input
                          type='checkbox'
                          checked={selectedConcepts.includes(item.name)}
                          onChange={() => toggleConcept(item.name)}
                          className='text-[#d52727] rounded'
                        />
                        <span className='flex-1'>{item.name}</span>
                        {item.count != null && (
